...
mod tests {
    ...
    fn t110_skip_only_fixture_produces_info() {
        ...
        let source = fixture("t110_violation.php");
        let extractor = PhpExtractor::new();
        let funcs = extractor.extract_test_functions(&source, "t110_violation.php");
        let diags: Vec<_> = evaluate_rules(&funcs, &Config::default())
            .into_iter()
    ...
    #[test]
    fn t110_mark_test_incomplete_path_produces_info() {
        use exspec_core::rules::{evaluate_rules, Config, RuleId, Severity};
        ...
        let diags: Vec<_> = evaluate_rules(&funcs, &Config::default())
            .into_iter()
            .filter(|d| {
                d.rule == RuleId::new("T110")
                    && d.severity == Severity::Info
                    && d.message.contains("testIncompleteFeature")
            })
            .collect();
        assert_eq!(
            diags.len(),
            1,
            "Expected markTestIncomplete path to emit one T110 INFO: {diags:?}"
        );
    }
