...
### Internal dogfooding results (2026-03-26)

3 private projects (anonymized A/B/C) で dogfooding 実施。lint FP と observe FN を発見・修正。

| Project | Lang | Lint Before | Lint After | Observe R Before | Observe R After |
|---------|------|-------------|------------|------------------|-----------------|
| A | TS/Next.js | BLOCK 1, WARN 21 | BLOCK 1, WARN 1 | 49% | **85%** (100% of mappable) |
| B | PHP/Laravel | BLOCK 0, WARN 211 | BLOCK 0, WARN 39 | 100% | 100% |
| C | Python/Django | BLOCK 0, WARN 0 | BLOCK 0, WARN 0 | 6% | **44%** |

**Patches shipped:**

| PR | Fix | Impact |
|----|-----|--------|
| #204 | T003 language-specific default thresholds (PHP=100, TS=75, Rust=100, Python=50) | PHP WARN 211→39, TS WARN 21→1. Aligned with PHPMD/clippy/ESLint defaults |
| #205 | Python observe `manage.py` root detection | Django project R 6%→44%. Resolves absolute imports relative to manage.py directory |
| #206 | TS observe dynamic import extraction | Next.js/Vitest R 49%→85%. `await import('@/lib/...')` pattern now captured |

**Remaining observations:**
- A (TS): BLOCK 1 is valid (assertion-free TCP test). 11 unmapped tests are all infrastructure (no production imports). Effectively R=100%.
- B (PHP): 39 WARNs are genuine T003 (101-174 lines). PHPMD-aligned threshold working as intended. No exspec fix needed.
- C (Python): R=44% ceiling likely from tests with no production imports + some Django-specific patterns (views.py resolution). Further investigation deferred.

**Decision**: Internal dogfooding v0.5.x P1 complete for initial pass. Major FP/FN resolved. Remaining items are either valid detections or diminishing-return improvements. Next: `exspec init` (P2 onboarding) or crates.io v0.5.1 release.
